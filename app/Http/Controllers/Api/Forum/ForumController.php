...
class ForumController extends Controller
{
    ...
    public function createReplyComment(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $new_reply_comment = ForumComment::create($comment);

            return response()->json(new ValueMessage(['value'=>1,'message'=>'Post Reply Comment Success!','data'=> $new_reply_comment]), 200);
        }
    }
    ...
    public function showReplyComment (Request $request){
        ...
        if ($validator->fails()) {
            ...
            $post_comment = ForumComment::where('id_reply', $request->id_reply)->get();
