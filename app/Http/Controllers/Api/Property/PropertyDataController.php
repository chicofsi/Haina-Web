            ...
            if(!$property){
                ...
                if($request->transaction_type == "buy" && $property['selling_price'] == 0){
                    ...
                    try{
                        ...
                        NotificationController::createNotif($property['id_user'], "Someone is interested with your property!", $buyername." did a ".$request->transaction_type." transaction with ".$property['name'], 7, 4);
        ...
        if ($validator->fails()) {
            ...
            if(!$transaction){
                ...
                if($request->status == "done"){
                    ...
                    NotificationController::createNotif($property['id_user'], "Your transaction is finished", "Transaction for ".$property['title']." is finished", 7, 1);
                    foreach($token as $key => $value) {
                        NotificationController::sendPush($property['id_user'], $value, "Your transaction is finished", "Transaction for ".$property['title']." is finished", "Property", "");
                    }
                    ...
                    NotificationController::createNotif($property['id_user'], "Your transaction is cancelled", "Transaction for ".$property['title']." is cancelled", 7, 2);
                    foreach($token as $key => $value) {
                        NotificationController::sendPush($property['id_user'], $value, "Your transaction is cancelled", "Transaction for ".$property['title']." is cancelled", "Property", "");
                    }
